...
class OffsetRastrigin(Problem):
    def __init__(self, d: int = 25, A: int = 10):
        ...
        self._x_prime = self.make_gaussian(
            d, center=0.0, stdev=1.0
        )  # Generate a random offset with center 0 and standard deviation 1

    def _evaluate(self, solution: Solution):
        x = solution.values
        z = x - self._x_prime
        f = (self._A * self.solution_length) + torch.sum(z.pow(2.0) - self._A * torch.cos(2 * math.pi * z))
...
class VecOffsetRastrigin(Problem):
    def __init__(self, d: int = 25, A: int = 10):
        ...
        self._A = A  # Store the A parameter for evaluation
        self._x_prime = self.make_gaussian(
            (1, d), center=0.0, stdev=1.0
        )  # Generate a random offset with center 0 and standard deviation 1

    def _evaluate_batch(self, solutions: SolutionBatch):
        xs = solutions.values
        zs = xs - self._x_prime
        fs = (self._A * self.solution_length) + torch.sum(zs.pow(2.0) - self._A * torch.cos(2 * math.pi * zs), dim=-1)
